...
<template>
  <div
    class="bg-white/10 backdrop-blur-lg rounded-2xl p-4 sm:p-6 border border-white/20 mb-8"
  >
    <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
      ...
      <span
        v-else
        class="text-green-400 text-sm flex items-center gap-1 whitespace-nowrap"
      >
        <svg
          class="w-4 h-4"
          fill="none"
    ...
    <div
      ...
      class="bg-yellow-500/20 border border-yellow-500/50 rounded-lg p-4 mb-6"
    >
    ...
    >
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-white/80 text-sm">
            Для получения уведомлений о событиях необходимо дать согласие на
            обработку данных.
          </p>
        </div>
      </div>
    ...
    <div v-else class="space-y-6">
        ...
        <div class="bg-white/5 rounded-lg p-3 sm:p-4 mb-3">
          ...
          >
            <div class="flex-1">
              ...
              <p class="text-white/40 text-xs hidden sm:block">
                Уведомление при добавлении нового события
              </p>
            </div>
            <div class="flex flex-wrap gap-3 sm:gap-4">
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_new_events_email"
                  class="toggle"
                />
              </label>
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_new_events_push"
                  class="toggle"
                  :disabled="!hasPhone"
                />
              </label>
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_new_events_webpush"
                  class="toggle"
                />
              </label>
            </div>
          </div>
          <p class="text-white/40 text-xs mt-2 sm:hidden">
            Уведомление при добавлении нового события
          </p>
        </div>
          ...
          >
            ...
            <div class="flex flex-wrap gap-3 sm:gap-4">
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_event_day_email"
                  class="toggle"
                />
              </label>
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_event_day_push"
                  class="toggle"
                  :disabled="!hasPhone"
                />
              </label>
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_event_day_webpush"
                  class="toggle"
                />
              </label>
            </div>
        ...
        <div class="bg-white/5 rounded-lg p-3 sm:p-4 mb-3">
          ...
          >
            <div class="flex-1">
              ...
              <p class="text-white/40 text-xs hidden sm:block">
                Уведомление за 2 часа до начала
              </p>
            </div>
            <div class="flex flex-wrap gap-3 sm:gap-4">
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_event_reminder_email"
                  class="toggle"
                />
              </label>
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_event_reminder_push"
                  class="toggle"
                  :disabled="!hasPhone"
                />
              </label>
              <label class="flex items-center gap-2">
                ...
                <input
                  type="checkbox"
                  v-model="settings.notify_event_reminder_webpush"
                  class="toggle"
                />
              </label>
            </div>
      ...
      <div class="bg-white/5 rounded-lg p-4 border border-white/10">
        ...
        >
          <div>
            ...
            <p v-if="pushPermissionDenied" class="text-red-400 text-xs mt-1">
              ⚠️ Разрешение на уведомления заблокировано. Проверьте настройки
              браузера.
            </p>
          </div>
          <div class="flex flex-col sm:flex-row gap-2">
            <button
              v-if="
                !isPushSubscribed && isPushSupported && !pushPermissionDenied
              "
        ...
        <div
          ...
          class="mt-2 text-yellow-400 text-xs"
        >
    ...
    >
      <div
        class="bg-white/10 backdrop-blur-lg p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-md border border-white/20"
      >
        ...
        <div class="mb-6">
          <label class="flex items-start gap-3">
            ...
            <span class="text-white/80 text-sm">
              Я согласен на обработку моих контактных данных для целей
              информирования о событиях
            </span>
          </label>
        </div>
...
const loadSettings = async () => {
  ...
  try {
    // ✅ Используем $api вместо $fetch
    const response = await $api<SettingsResponse>('/user/notification-settings');

    if (response.success && response.settings) {
      settings.value = {
        notify_new_events_email:
          response.settings.notify_new_events_email || false,
        notify_new_events_push:
          response.settings.notify_new_events_push || false,
        notify_new_events_webpush:
          response.settings.notify_new_events_webpush || false,
        notify_event_reminder_email:
          response.settings.notify_event_reminder_email || false,
        notify_event_reminder_push:
          response.settings.notify_event_reminder_push || false,
        notify_event_reminder_webpush:
          response.settings.notify_event_reminder_webpush || false,
        notify_event_day_email:
          response.settings.notify_event_day_email || false,
        notify_event_day_push: response.settings.notify_event_day_push || false,
        notify_event_day_webpush:
          response.settings.notify_event_day_webpush || false,
      };
      phoneForNotifications.value =
        response.settings.phone_for_notifications || "";
      hasConsent.value = response.settings.has_consent || false;
      notificationsAvailable.value =
        response.settings.notifications_available !== false;
...
const testNotification = async (channel: "email" | "push" | "webpush") => {
  ...
  if (channel === "push" && !hasPhone.value) {
    notificationStore.warning(
      "Нет номера",
      "Укажите номер телефона для теста SMS",
    );
    return;
  }
  ...
  try {
    // ✅ Используем $api вместо $fetch
    const response = await $api<TestResponse>('/user/test-notification', {
      method: "POST",
...
// Преобразование для applicationServerKey
...
const enablePushNotifications = async () => {
  ...
  if (!("PushManager" in window)) {
    notificationStore.warning(
      "Не поддерживается",
      "Push Manager не поддерживается",
    );
    isPushSupported.value = false;
    pushLoading.value = false;
    return;
  ...
  try {
    ...
    // ✅ Используем $api вместо $fetch
    const response = await $api<PushSubscriptionResponse>('/push-subscription', {
      method: "POST",
      body: {
    ...
    if (response.success) {
      ...
      notificationStore.success(
        "Web Push включены",
        "Теперь вы будете получать уведомления в браузере",
      );
      await saveSettings();
    } else {
      notificationStore.error(
        "Ошибка",
        response.message || "Не удалось сохранить подписку",
      );
    }
  } catch (err: any) {
    ...
    if (err?.message?.includes("permission")) {
      ...
      notificationStore.error(
        "Ошибка",
        err.message || "Не удалось включить Web Push",
      );
    }
  } finally {
    pushLoading.value = false;
  ...
  try {
    ...
    if (subscription) {
      ...
      if (unsubscribed) {
        ...
        try {
          // ✅ Используем $api вместо $fetch
          await $api<PushSubscriptionResponse>('/push-subscription', {
            method: "DELETE",
    ...
    notificationStore.error(
      "Ошибка",
      err.message || "Не удалось отключить Web Push",
    );
  } finally {
    pushLoading.value = false;
  }
