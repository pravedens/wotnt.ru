<template>
  <div
    class="relative group rounded-2xl overflow-hidden shadow-xl transition-all duration-300 hover:scale-105 hover:shadow-2xl h-full flex flex-col bg-transparent"
  >
    <!-- Фоновое изображение -->
    <img
      v-if="imageUrl"
    ...
    <div class="relative flex flex-col h-full p-3 z-10">
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2">
          <ViewsCount :views="postStats.views_count" />
          <LikeButton
            :post-id="post.id"
            :initial-liked="postStats.liked"
            :initial-likes-count="postStats.likes_count"
          />
        </div>
      </div>
...
// ✅ Статистика поста с дефолтными значениями
const postStats = computed(() => ({
  views_count: props.stats?.views_count ?? 0,
  likes_count: props.stats?.likes_count ?? 0,
  liked: props.stats?.liked ?? false,
}));

// ✅ Очищенное описание
const cleanDescription = computed(() => {
  const text =
    props.post.clean_description ||
