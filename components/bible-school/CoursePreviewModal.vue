<template>
  ...
  >
    <div
      class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-white/20"
    >
      <img
      ...
      <div class="p-6">
        ...
        <div
          ...
          v-sanitize-html="course.description"
        ></div>
        ...
        <div v-if="filteredThemes.length" class="mb-6">
          ...
          <div class="space-y-4">
            ...
            >
              <div
                class="flex justify-between items-start flex-wrap gap-2 mb-2"
              >
                ...
                <div
                  v-if="theme.teacher"
                  class="flex items-center gap-2 bg-white/10 rounded-full px-3 py-1"
                >
                  <img
                    :src="theme.teacher.avatar_url"
                    class="w-6 h-6 rounded-full"
                  />
                  <span class="text-white/80 text-sm">{{
                    theme.teacher.full_name
                  }}</span>
                </div>
              </div>
              <p
                v-if="theme.description"
                class="text-white/60 text-sm mb-3"
              ...
              <div class="flex items-center gap-2 text-white/40 text-sm">
                <span
                  ...
                  {{
                    getLessonsCountText(
                      theme.lessons_count || theme.lessons?.length || 0,
                    )
                  }}</span
                >
              </div>
        ...
        <div v-if="statuses.length" class="mb-6 bg-white/5 rounded-xl p-4">
          ...
          <div class="flex flex-wrap gap-3">
            <div
              ...
              class="px-4 py-2 bg-white/10 rounded-full text-white/80 text-sm"
            >
        ...
        <!-- 3. Что вы узнаете -->
        <div
          v-if="course.what_you_will_learn"
          class="mb-6 p-4 bg-blue-500/10 rounded-xl border border-blue-500/20"
        >
          ...
          <div
            class="text-white/70"
            v-sanitize-html="course.what_you_will_learn"
          ></div>
        </div>
        ...
        <div
          v-if="course.skills"
          class="mb-6 p-4 bg-green-500/10 rounded-xl border border-green-500/20"
        >
          <h3 class="text-lg font-semibold text-white mb-2">
            💡 Практические навыки
          </h3>
        ...
        <!-- 5. Сертификат -->
        <div
          class="mb-6 text-center p-4 bg-yellow-500/10 rounded-xl border border-yellow-500/20"
        >
          <p class="text-white/80 text-sm">
            🎓 По окончании курса вы получите именной сертификат
          </p>
          <p class="text-white/50 text-xs mt-1">
            {{
              course.certificate_text ||
              "Успешно завершил(а) полный курс обучения"
            }}
          </p>
        </div>
...
const fetchCourseDetails = async () => {
  try {
    ...
    const response = await $api(
      `/bible-school/courses/${props.course.slug}/preview`,
    );
