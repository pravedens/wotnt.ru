    ...
    <div class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 max-w-md w-full border border-white/20">
      ...
      <div v-if="!isAuthenticated" class="space-y-4">
        <div>
          ...
          <input v-model="form.name" type="text" class="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white" placeholder="Иван">
        </div>
        <div>
          <label class="block text-white/80 text-sm mb-1">Ваш Email <span class="text-red-400">*</span></label>
          <input v-model="form.email" type="email" class="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white" placeholder="ivan@example.com">
        </div>
        <div>
          <label class="block text-white/80 text-sm mb-1">Сообщение <span class="text-red-400">*</span></label>
          <textarea v-model="form.message" rows="4" class="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white" placeholder="Введите ваше сообщение..."></textarea>
        </div>
      ...
      <div v-else class="space-y-4">
        <div>
          <label class="block text-white/80 text-sm mb-1">Сообщение <span class="text-red-400">*</span></label>
          <textarea v-model="form.message" rows="5" class="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white" placeholder="Введите ваше сообщение..."></textarea>
        </div>
...
const sendMessage = async (): Promise<void> => {
  ...
  try {
    ...
    await $fetch(`/api/teachers/${props.teacher.id}/message`, {
      method: 'POST',
