<template>
  <div class="relative" v-if="isMinister">
    <button
      @click="openMessages"
      class="relative p-2 text-white/80 hover:text-white transition"
    >
      <svg
        class="w-6 h-6"
        fill="none"
    ...
    <div
      v-if="showDropdown"
      class="absolute right-0 mt-2 w-80 bg-gray-800 rounded-xl shadow-xl border border-gray-700 z-50"
    >
      ...
      <div
        ...
        class="p-4 text-center text-white/40"
      >
        Нет новых сообщений
      </div>
      <div v-else class="max-h-96 overflow-y-auto">
        <div
          ...
          :key="msg.id"
          class="p-3 hover:bg-white/5 cursor-pointer border-b border-gray-700"
          @click="goToMessages"
        >
          ...
          <p
            class="text-white/60 text-sm truncate"
      ...
      <div class="p-2 border-t border-gray-700">
        <button
          @click="goToMessages"
          class="w-full text-center text-blue-400 text-sm py-1 hover:text-blue-300"
        >
          Все сообщения →
        </button>
      </div>
