<template>
  <Teleport to="body">
    ...
    >
      <!-- Затемнение -->
      <div
        class="absolute inset-0 bg-black/50 backdrop-blur-sm animate-fadeIn"
      ></div>

      <!-- Модальное окно -->
      <div
        class="relative bg-gradient-to-br from-blue-900 to-purple-900 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden border border-white/20 shadow-2xl animate-slideUp"
      >
        <!-- Заголовок -->
        ...
        <div class="p-4 sm:p-6 overflow-y-auto max-h-[55vh] sm:max-h-[60vh]">
          <div v-if="events.length" class="space-y-3 sm:space-y-4">
            ...
            >
              <div class="flex gap-3 sm:gap-4">
                ...
                >
                  <svg
                    class="w-6 h-6 sm:w-8 sm:h-8 text-white/60"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <h3
                    class="text-sm sm:text-lg font-semibold text-white group-hover:text-blue-300 transition line-clamp-2"
                  >
                    {{ event.title }}
                  </h3>
                  <p
                    v-if="event.description"
                    class="text-white/60 text-xs sm:text-sm line-clamp-1 mt-1"
                    v-sanitize-html="event.description"
                  ></p>
                </div>

                <!-- Стрелка -->
                <svg
                  class="w-4 h-4 sm:w-5 sm:h-5 text-white/40 flex-shrink-0 group-hover:text-white/70 transition"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <div v-else class="text-center py-8 sm:py-12 text-white/60">
            <svg
              class="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 text-white/30"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
