...
>
<div
class="relative rounded-xl sm:rounded-2xl overflow-hidden border border-white/20 hover:border-white/40 transition-all"
>
...
loading="lazy"
>
<div v-else class="w-full h-full bg-gradient-to-br from-blue-500 to-purple-500"></div>
...
<p v-if="events[currentIndex].description && !isMobile"
class="text-white/70 text-xs sm:text-sm line-clamp-2 max-w-2xl mb-1 sm:mb-3">
...
// Класс контейнера изображения
const imageContainerClass = computed(() => {
  return isMobile.value ? 'h-64' : 'aspect-video'
})
...
const loadUpcomingEvents = async () => {
  ...
  try {
    const response = await $fetch(
      'https://wotgospel.ru/api/events/upcoming',
      {
