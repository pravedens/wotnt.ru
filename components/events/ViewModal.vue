<template>
  <Teleport to="body">
    ...
    >
      ...
      <!-- Модальное окно -->
      <div
        class="relative bg-gradient-to-br from-blue-900 to-purple-900 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden border border-white/20 shadow-2xl"
      >
        ...
        <div
          ...
          :class="{
            'max-h-[calc(90vh-12rem)]': imageUrl,
            'max-h-[90vh]': !imageUrl,
          }"
        >
          <!-- Заголовок и кнопка закрытия -->
          <div class="flex justify-between items-start mb-4">
            <h2 class="text-2xl font-bold text-white pr-8">
              {{ event?.title || "Событие" }}
            </h2>
            <button
              @click="$emit('close')"
          ...
          <div
            v-if="event?.location"
            class="flex items-center gap-2 text-white/60 mb-4"
          >
            <svg
              class="w-5 h-5 flex-shrink-0"
              fill="none"
