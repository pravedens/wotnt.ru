<template>
  <div class="space-y-4 md:space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
      <div
        class="bg-white/10 backdrop-blur-lg rounded-xl md:rounded-2xl p-3 md:p-4 text-center border border-white/20"
      >
    ...
    <div class="flex flex-wrap gap-2 md:gap-3 pb-4 border-b border-white/20">
      <NuxtLink
        to="/teacher/courses"
        class="px-3 py-1.5 md:px-4 md:py-2 bg-purple-500/80 text-white rounded-lg hover:bg-purple-600 transition text-xs md:text-sm"
      >
        📚 Курсы
      </NuxtLink>
      <NuxtLink
    ...
    <!-- ========== СПИСОК СТУДЕНТОВ ========== -->
    <div
      v-show="activeTeacherTab === 'students'"
      class="bg-white/10 backdrop-blur-lg rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20"
    >
      <div
        class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mb-4"
      >
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          ...
          <select
            ...
            class="px-3 py-1.5 bg-white/10 border border-white/20 rounded-lg text-white text-sm"
          >
            <option class="bg-gray-800 text-white" :value="null">
              Все годы
            </option>
            <option
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-2 md:py-3 font-medium">
                ...
                <div class="text-white/40 text-xs md:hidden">
                  {{ student.email }}
                </div>
              </td>
              <td class="py-2 md:py-3 hidden md:table-cell">
                {{ student.email }}
              </td>
              <td class="py-2 md:py-3">
                <div class="flex items-center gap-1 md:gap-2">
                  <div
                    class="w-16 md:w-24 bg-white/20 rounded-full h-1.5 md:h-2"
                  >
                    <div
                      class="bg-blue-500 h-1.5 md:h-2 rounded-full"
                      :style="{
                        width: (student.progress?.percentage || 0) + '%',
                      }"
                    ></div>
                  </div>
                  <span class="text-xs"
                    >{{ student.progress?.percentage || 0 }}%</span
                  >
                </div>
              </td>
    ...
    <!-- ========== ЭССЕ НА ПРОВЕРКУ ========== -->
    <div
      v-show="activeTeacherTab === 'essays'"
      class="bg-white/10 backdrop-blur-lg rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20"
    >
      ...
      <div
        v-else-if="pendingEssays.length === 0"
        class="text-center py-8 text-white/60"
      >
      ...
      <div v-else class="space-y-3 md:space-y-4">
        <div
          v-for="essay in pendingEssays"
          :key="essay.id"
          class="bg-white/5 rounded-lg md:rounded-xl p-3 md:p-4 border border-white/10"
        >
          <div class="flex flex-wrap justify-between items-start gap-2 mb-2">
            <div>
              <p class="text-white font-semibold text-sm md:text-base">
                {{ essay.user?.full_name || essay.user?.name }}
              </p>
              <p class="text-white/60 text-xs">
                Урок: {{ essay.lesson?.title }}
              </p>
            </div>
            <button
          ...
          <div
            class="bg-white/5 rounded-lg p-2 md:p-3 max-h-24 md:max-h-32 overflow-y-auto"
          >
            <p
              v-sanitize-html="essay.content"
              class="text-white/70 text-xs md:text-sm whitespace-pre-wrap"
            ></p>
          </div>
        </div>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
        ...
        >
          <div class="flex items-center gap-2 md:gap-3">
            ...
            <div class="min-w-0">
              <p
                class="text-white font-medium text-sm md:text-base truncate max-w-[120px] md:max-w-none"
              >
                {{ student.full_name }}
              </p>
              <p
                class="text-white/40 text-xs truncate max-w-[100px] md:max-w-none"
              >
                {{ student.email }}
              </p>
            </div>
    ...
    <div
      v-show="activeTeacherTab === 'enrollments'"
      class="bg-white/10 backdrop-blur-lg rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20"
    >
      <div class="flex justify-between items-center mb-4">
      ...
      <div
        v-else-if="enrollmentRequests.length === 0"
        class="text-center py-8 text-white/60"
      >
        Нет новых заявок
      </div>
      ...
      <div v-else class="space-y-3 md:space-y-4">
        <div
          ...
          :key="request.id"
          class="bg-white/5 rounded-lg md:rounded-xl p-3 md:p-5 border border-white/10"
        >
          <div
          ...
          >
            <div class="flex-1">
              ...
              <div
                class="grid grid-cols-2 gap-1 md:gap-2 mt-2 text-xs md:text-sm"
              >
                <p>
                  <span class="text-white/40">Город:</span>
                  {{ request.city || "—" }}
                </p>
                <p>
                  <span class="text-white/40">Церковь:</span>
                  {{ request.church_name || "—" }}
                </p>
                <p class="col-span-2">
                  <span class="text-white/40">Служение:</span>
                  {{ request.ministry || "—" }}
                </p>
              </div>
              <p
                v-if="request.about"
                class="mt-2 text-white/60 text-xs line-clamp-2"
              >
                📝 {{ request.about }}
              </p>
            </div>
            <div class="flex flex-col sm:flex-row gap-2">
              <select
                v-model="selectedCourseForRequest[request.id]"
                class="px-2 py-1.5 bg-white/10 border border-white/20 rounded-lg text-xs md:text-sm"
              >
    ...
    <!-- ========== ОТКЛОНЁННЫЕ ЗАЯВКИ ========== -->
    <div
      v-show="activeTeacherTab === 'rejected'"
      class="bg-white/10 backdrop-blur-lg rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20"
    >
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg md:text-xl font-bold text-white">
      ...
      <div
        v-else-if="rejectedRequests.length === 0"
        class="text-center py-8 text-white/60"
      >
...
const updateStudentRole = async (userId, role) => {
  ...
  try {
    ...
    notificationStore.success(
      "Роль обновлена",
      `Пользователь теперь ${role === "group_leader" ? "лидер группы" : "ученик"}`,
    );
...
const unblockUser = async (userId) => {
  ...
  if (!confirm("Разблокировать пользователя? Он сможет подать новую заявку."))
    return;
...
const openStudentChat = async (student) => {
  ...
  try {
    const response = await $api(
      "/bible-school/chat/conversations/find-or-create",
      {
        method: "POST",
