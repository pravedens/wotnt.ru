  ...
  >
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :class="
          msg.is_read ? 'border-white/10' : 'border-blue-500/50 bg-blue-500/5'
        "
      >
        ...
        <div class="cursor-pointer" @click="toggleMessage(msg.id)">
          <p
            v-sanitize-html="msg.message"
            class="text-white/80 text-sm whitespace-pre-wrap"
            :class="{ 'line-clamp-2': !expandedMessages[msg.id] }"
          ></p>
        </div>

        <div
          class="flex justify-between items-center mt-3 pt-3 border-t border-white/10"
        >
          <button
...
const deleteMessage = async (messageId: number) => {
  ...
  try {
    ...
    notificationStore.error(
      "Ошибка",
      error?.data?.message || "Не удалось удалить сообщение",
    );
  }
};
