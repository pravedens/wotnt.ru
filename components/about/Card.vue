<template>
  ...
  >
    <div
      class="relative h-48 overflow-hidden bg-gradient-to-br from-blue-600 to-purple-600"
    >
      <img
        v-if="imageUrl"
        :src="imageUrl"
      ...
      <div class="absolute bottom-4 left-4 z-10">
        <span
          class="px-3 py-1 bg-blue-500/80 text-white text-sm rounded-full backdrop-blur-sm flex items-center gap-1"
        >
          <span class="text-sm">{{
    ...
    <div class="p-6 flex-1 flex flex-col">
      <h3
        class="text-xl font-bold text-white mb-2 group-hover:text-blue-300 transition line-clamp-2"
      >
        {{ about.title }}
      </h3>
      ...
      <p
        ...
        class="text-white/70 line-clamp-3 mb-4 flex-1"
      ></p>
      ...
      >
        ...
        <span
          class="text-blue-400 group-hover:text-blue-300 transition flex items-center gap-1"
        >
          Читать далее
          <svg
...
const imageUrl = computed(() =>
  props.about.thumbnail ? getImageUrl(props.about.thumbnail, "abouts") : null,
);
