<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-900 to-purple-900 py-8 md:py-12">
    <div class="container mx-auto px-4">
      <AboutBreadcrumbs />
      
      <h1 class="text-3xl md:text-4xl font-bold text-white text-center mb-6 md:mb-8">
        О нас
      </h1>
      ...
      <div v-else-if="error" class="bg-red-500/20 border border-red-500/50 rounded-2xl p-8 text-center max-w-2xl mx-auto">
        ...
        <button @click="reload" class="px-6 py-2 bg-red-500/30 text-red-200 rounded-lg hover:bg-red-500/40 transition">
          Попробовать снова
        </button>
      </div>
      ...
      <div v-else class="max-w-4xl mx-auto">
        ...
        <div class="flex flex-wrap justify-center gap-2 mb-8">
          <button
            ...
            class="category-btn px-4 py-1.5 text-sm rounded-full font-medium transition-all duration-200"
            :class="[
          ...
          >
            <span class="mr-1 text-base">{{ getCategoryIcon(denom.title) }}</span>
            {{ denom.title }}
        ...
        <div class="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 overflow-hidden">
          <!-- Загрузка -->
          <div v-if="loadingAbouts" class="flex flex-col items-center justify-center py-16">
            <div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-white border-t-transparent"></div>
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center justify-between mt-2">
                          ...
                          <span class="text-blue-400 group-hover:text-blue-300 transition flex items-center gap-1 text-xs">
                            Читать
                            <svg class="w-3 h-3 group-hover:translate-x-0.5 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const selectDenomination = async (denom: Denomination) => {
  if (selectedDenomination.value?.id === denom.id || loadingAbouts.value) return
