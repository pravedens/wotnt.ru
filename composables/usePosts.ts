...
export const usePosts = () => {
  ...
  const loadFilterData = async () => {
    try {
      const [categoriesRes, groupsRes, conferencesRes] = await Promise.all([
        ...
        $fetch<Conference[]>('/api/conferences')
      ])
  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      params.append('per_page', String(filters.value.per_page || 8))
      ...
      const response = await $fetch<PaginatedResponse<Post>>(`/api/posts?${params.toString()}`)
