  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-900 to-purple-900 py-12">
    <div class="container mx-auto px-4">
      ...
      <div v-if="isTeacher && !isSuperAdmin" class="mb-8 max-w-2xl mx-auto">
        <div
          class="bg-green-500/20 border border-green-500/50 rounded-2xl p-6 text-center"
        >
          <p class="text-green-400 text-lg mb-2">
            👨‍🏫 Вы вошли как преподаватель
          </p>
          <p class="text-white/70 mb-4">
            Управляйте студентами, проверяйте эссе и отслеживайте прогресс
          </p>
          <NuxtLink
      ...
      >
        <div
          ...
          :key="course.id"
          class="bg-white/10 backdrop-blur-lg rounded-2xl overflow-hidden border border-white/20 hover:bg-white/15 transition-all group flex flex-col"
        >
          <img
          ...
          <div
            v-else
            class="w-full h-48 bg-gradient-to-r from-blue-500/30 to-purple-500/30 flex items-center justify-center"
          >
          ...
          <div class="p-6 flex flex-col flex-1">
            ...
            <div class="mt-4">
              ...
              <div
                v-else-if="pageData.enrollment_status === 'pending'"
                class="text-center py-2 text-yellow-400"
              >
                ⏳ Заявка на рассмотрении
              </div>
      ...
      <div
        class="mt-16 bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20"
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="text-center">
            ...
            <p class="text-white/60 text-sm">
              Нажмите «Хочу учиться» на выбранном курсе
            </p>
          </div>
          <div class="text-center">
            ...
            <p class="text-white/60 text-sm">
              Проходите уроки, сдавайте тесты и получайте сертификаты
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        class="mt-16 bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20"
      >
        <h2 class="text-2xl font-bold text-white text-center mb-6">
          🎓 Наши выпускники
        </h2>
        ...
        >
          <div
            ...
            :key="grad.id"
            class="bg-white/5 rounded-xl p-3 text-center"
          >
            <img
              :src="grad.avatar_url"
...
const filteredGraduates = computed(() => {
  ...
  return graduates.value.filter(
    (g) => g.graduation_year === selectedYear.value,
  );
});
...
const repeatCourse = async (course) => {
  if (
    confirm(
      "Вы уверены, что хотите повторить курс? Весь прогресс будет сброшен.",
    )
  ) {
    try {
      // ✅ Используем $api вместо $fetch
