  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-900 to-purple-900">
    <div class="container mx-auto px-4 py-12">
      ...
      <p class="text-white/80 text-center mb-8 max-w-2xl mx-auto">
        Команда служителей, которые заботятся о церкви и помогают людям расти в
        вере
      </p>
      ...
      >
        ...
        <AppButton
          ...
          :key="cat.id"
          :variant="
            selectedCategory?.id === cat.id ? 'category-active' : 'category'
          "
      ...
      <div
        v-else-if="!loading && allCategories.length > 0"
        class="text-center text-white/60 py-8 mb-12"
      >
      ...
      <div
        v-else-if="filteredMinisters.length === 0"
...
const filteredMinisters = computed(() => {
  ...
  return allMinisters.value.filter((minister) =>
    minister.minister_categories?.some(
      (cat: any) => cat.id === selectedCategory.value.id,
    ),
  );
});
...
onMounted(async () => {
  loading.value = true;
  try {
    const [ministers, categories] = await Promise.all([
      getMinisters(),
      getAllCategories(),
    ]);
